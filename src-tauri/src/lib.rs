...
fn save_runtime_target(
    ...
    if existing.is_none() && token.is_empty() {
        return Err("新建目标需要填写 Token。".into());
    }

    if !token.is_empty() {
        set_token(&app, &id, token)?;
    }
...
#[tauri::command]
fn get_runtime_target_token(app: AppHandle, id: String) -> Result<String, String> {
    get_token(&app, &id)
}
...
fn token_entry(id: &str) -> Result<Entry, String> {
    Entry::new(KEYRING_SERVICE, id).map_err(|err| {
        format!("无法打开系统凭据存储（keyring）：{err}。将尝试应用私有文件回退。")
    })
}

fn token_fallback_path(app: &AppHandle, id: &str) -> Result<PathBuf, String> {
    let dir = app
        .path()
        .app_config_dir()
        .map_err(|err| format!("无法解析应用配置目录：{err}"))?;
    let tokens_dir = dir.join("tokens");
    fs::create_dir_all(&tokens_dir)
        .map_err(|err| format!("无法创建 Token 私有目录：{err}"))?;
    // Keep path segment simple; ids are generated by this app.
    let safe_id = id.replace(['/', '\\', ':', '*', '?', '"', '<', '>', '|'], "_");
    Ok(tokens_dir.join(format!("{safe_id}.token")))
}

fn set_token(app: &AppHandle, id: &str, token: &str) -> Result<(), String> {
    match token_entry(id) {
        Ok(entry) => match entry.set_password(token) {
            Ok(()) => return Ok(()),
            Err(err) => {
                // Fall through to private file on Android / unavailable keyring.
                let _ = err;
            }
        },
        Err(_) => {}
    ...
    let path = token_fallback_path(app, id)?;
    fs::write(&path, token.as_bytes())
        .map_err(|err| format!("系统凭据不可用，且无法写入私有 Token 文件：{err}"))?;
    Ok(())
}

fn get_token(app: &AppHandle, id: &str) -> Result<String, String> {
    ...
    let path = token_fallback_path(app, id)?;
    if !path.exists() {
        return Err("未找到已保存的 Token。请重新填写并保存连接。".into());
    }

    let text = fs::read_to_string(&path)
        .map_err(|err| format!("无法读取私有 Token 文件：{err}"))?;
    let trimmed = text.trim();
    if trimmed.is_empty() {
        return Err("已保存的 Token 为空。请重新填写并保存连接。".into());
    }
    Ok(trimmed.to_string())
}

fn delete_token(app: &AppHandle, id: &str) {
    if let Ok(entry) = token_entry(id) {
        let _ = entry.delete_credential();
    }
    if let Ok(path) = token_fallback_path(app, id) {
...
fn normalize_base_url(value: &str) -> Result<String, String> {
    ...
    if trimmed.is_empty() {
        return Err("Runtime 地址不能为空。".into());
    }

    if !(trimmed.starts_with("http://") || trimmed.starts_with("https://")) {
        return Err("Runtime 地址需以 http:// 或 https:// 开头。".into());
    }
